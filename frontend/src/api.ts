...
type SSEHandler = {
  ...
  onToolCall?: (data: import("./types").ToolCallEvent) => void;
  onGuardrailsSummary?: (data: import("./types").GuardrailsSummaryData) => void;
  ...
  while (true) {
    ...
    for (const part of parts) {
      ...
      try {
        ...
        if (event === "guardrails_summary") handlers.onGuardrailsSummary?.(parsed);
